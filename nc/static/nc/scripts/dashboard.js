(function() {
  ...
  $(document).ready(function() {
    ...
    Object.keys(CURRENT_DISPLAY).forEach(function(k) {
      setDisplayButtonType(k, CURRENT_DISPLAY[k]);
    });
  });
  ...
  // Set event listener to toggle the current top portfolio button display type
  $('.btn-leaderboard').on('click', function() {
    toggleDisplayButtonType('leaderboard');
  });

  function setDisplayButtonType(type, display) {
    /*
    Set the currently showing button type on top asset or performer list
    */
    if (ALLOWED_DISPLAYS[type].includes(display)) {
      // Fade out the old class
      let currentDisplayClass = '.btn-' + CURRENT_DISPLAY[type];
  ...
  function toggleDisplayButtonType(type) {
    ...
    */
    let displayIndex = ALLOWED_DISPLAYS[type].indexOf(CURRENT_DISPLAY[type]),
        newDisplayIndex = (displayIndex + 1) % ALLOWED_DISPLAYS[type].length,
        newDisplay = ALLOWED_DISPLAYS[type][newDisplayIndex];
    setDisplayButtonType(type, newDisplay);
  }
